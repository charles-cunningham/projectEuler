...
numbers0_9 <- c( "one", "two", "three", "four", "five",
                 "six", "seven", "eight", "nine", "ten")
numbers10_19 <- c( "eleven", "twelve", "thirteen", "fourteen", "fifteen",
                   "sixteen", "seventeen", "eighteen", "nineteen", "twenty")
numbers10s <- c( "", "ten", "twenty", "thirty", "forty",
                 "fifty", "sixty", "seventy", "eighty", "ninety", "onethousand")
...
for (i in startN:stopN) {
  
  ### Create word for number i
  
  # If 1-9...
  if (count10s == 1 & count100s == 1) { 
    iWord <- numbers0_9[count1s] }
  
  # If 10-19...
  if (count10s == 2 & count100s == 1) {
    iWord <- numbers10_19[count1s] }
  
  # If 20-99
  if (count10s > 2 & count100s == 1) {
    iWord <- paste0( numbers10s[count10s],
                     numbers0_9[count1s]) }
  
  # If multiple of 100
  if (count1s == 1 & count10s == 1 & count100s > 1 & count100s < 11) { 
    iWord <- paste0(numbers0_9[count100s],
                    "hundred") }
  
  # If 101-109, 201-209 etc
  if (count1s > 1 & count10s == 2 & count100s > 1 & count100s < 11) {
    iWord <- paste0( numbers0_9[count100s],
                     "hundredand",
                     numbers0_9[count1s]) }
  
  # If 110-119, 210-219 etc
  if (count10s == 3 & count100s > 1 & count100s < 11) {
    iWord <- paste0( numbers0_9[count100s],
                     "hundredand",
                     numbers10_19[count1s]) }
  
  # If 120-199, 220-299 etc
  if (count10s > 1 & count100s > 1 & count100s < 11) {
    iWord <- paste0( numbers0_9[count100s],
                     "hundredand", 
                     numbers10s[count10s],
                     numbers0_9[count1s]) }
  ...
  letterCount <- letterCount + nchar(iWord)

  # Update digit counters        
  if (count1s < 10) { # If 1s are under 9...
    count1s <- count1s + 1 } # ... add 1 to count1s
  else if (count10s < 10) { # Otherwise, if 10s are under 9 (i.e. 89 or less) ...
    count1s <- 1 # ... reset count1s to 0 ...
    count10s <- count10s + 1 } # ... and add 1 to count10s
  else { # Otherwise (if 1s and 10s are both 9, i.e. 99) ...
    count1s <- 1 # ... reset count1s to 0, ...
    count10s <- 1 # ... reset count10s to 0 ...
