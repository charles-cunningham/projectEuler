...
# Read in string as data.frame
xyGrid <- read.table(text = xyGridRaw, sep = " ", ) %>%
  as.matrix(.)

# Remove column names
colnames(xyGrid) <- NULL
...
for (i in 1 : NROW(xyGrid)) {
  ...
  for (j in 1 : NCOL(xyGrid)) {

  # Reset empty arrays for the adjacency arrays
  adjN <- adjNE <- adjE <- adjSE <- adjS <- adjSW <- adjW <- adjNW<- c()
  ...
  # EAST
  if (j <= (NCOL(xyGrid)  - numAdj + 1)) {
    
    adjE <- xyGrid[i, j : (j+numAdj-1)]
    
    }
  
  # SOUTHEAST
  if (i <= (NROW(xyGrid)  - numAdj + 1) & 
      j <= (NCOL(xyGrid)  - numAdj + 1)) {
    
    for (x in 0:(numAdj-1)) {
      
      adjSE <- c(adjSE, xyGrid[ i+x , j+x ])
  ...
  if (j >= numAdj) {
    ...
    adjW <- xyGrid[i , j: (j-numAdj +1)]
    
    }
  
  # NORTHWEST
  if (i >= numAdj & j >= numAdj) {
    
    for (x in 0:(numAdj-1)) {
      
      adjNW <- c(adjNW, xyGrid[ i-x , j-x ])
      
      }}
  
  # Combine all adjacent values in every direction into one list 
  adjAll <- list(adjN, adjNE, adjE, adjSE,
                 adjS, adjSW, adjW, adjNW)
  
  # Find product of adjacent values in every direction
  prodAll <- lapply(adjAll,prod) %>% 
    unlist(.)
  ...
  if (max(prodAll) > maxProd) {
    ...
    maxProd <- max(prodAll)
    maxCoord <- c(i,j)
    maxAdj <- adjAll[[which(prodAll == max(prodAll))]]
    
  }}}

# Return solution
print(paste("The greatest product is", maxProd,
            "starting at row", maxCoord[1],
            "and column", maxCoord[2], 
            "with values", list(maxAdj)))
