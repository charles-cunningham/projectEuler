...
while (nDiv <= nDivThresh):
    ...
    nCheck = list(range(1, int(pow(tNum, 1/2))))
    
    # Find values of nCheck which are divisors (i.e. tNum divisible by them)
    div = [x for x in nCheck if tNum % x == 0]
    
    # Find complementary values of div (i.e. divisors > square root)
    divPair = [tNum / x for x in div] 
    
    # Combine div and divPair and remove any duplicate divisors
    allDiv = set(div + divPair)
